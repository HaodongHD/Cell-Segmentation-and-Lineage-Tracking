...
Note: The experimental imaging data used in this repository is not included due to publication restrictions. Data can be provided upon reasonable request through the corresponding authors of the following paper:
Cislo, D.J., Yang, F., Qin, H. et al. (2023). Active cell divisions generate fourfold orientationally ordered phase in living tissue. Nature Physics 19, 1201–1210. https://doi.org/10.1038/s41567-023-02025-3

## 📦 Overview

The workflow consists of two main stages:

1. **Segmentation** – Converts ilastik probability maps into clean binary and labeled masks, extracts cell centroids, and optionally saves overlay images.
2. **Tracking** – Links centroids across time points to build lineage maps and optionally visualizes trajectories over the raw image stack.
...
* `R_seg.labels_TYX` — 3D label arrays (time × height × width)
* `R_seg.centroids_um` — per-frame cell centroids in µm
* `R_seg.count_total`, `R_seg.count_roi` — cell counts over time
...
* `R_track.cell_lineage_index` — lineage table (lineage × time)
* `R_track.index_adjacent_time` — parent-child mapping arrays
* `R_track.colors_rgb` — unique lineage colors
